...
pub struct DvocController {
    ...
    ki: Param, // Base current (A)
    pub kv: Param, // Base voltage (V)
    xi: Param,
    c: Param,  // Oscillator capacitance (F)
    l: Param,  // Oscillator inductance (H)
...
impl Dynamics<DVOC_STATES, DVOC_INPUTS> for DvocController {
    ...
    fn dynamics(&self, x:  &DvocStates, u: [Input; DVOC_INPUTS]) -> DvocStates {
        let (v, theta) = (x[0], x[1]);
