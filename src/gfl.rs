...
/// A grid-following (GFL) controller
pub struct GflController<'a, T: Num, const P: usize> {
    ...
    pub v: T,  // stored output voltage magnitude (p.u.)
    pub theta: T, // stored output voltage angle (p.u.)
...
impl<'a, const P: usize> GflController<'a, f32, P> {
    /// Constructs a grid-following controller from the given controller parameters
    /// # Arguments
    /// * 'v_nom' - nominal voltage (V)
    /// * 'w_nom' - nominal frequency (rad/s)
    /// * 'kp_d' - direct-axis proportional gain
    /// * 'ki_d' - direct-axis integral gain
    /// * 'kp_q' - quadrature-axis proportional gain
    /// * 'ki_q' - quadrature-axis integral gain
    /// * 'l' - filter inductance (p.u.)
    /// * 'pll' - reference to a phase-lock loop object
    /// * 'n_phase' - number of electrical phases of the inverter
    pub fn new(v_nom: f32, w_nom: f32, kp_d: f32, ki_d: f32, kp_q: f32, ki_q: f32, l: f32, pll: &'a mut dyn PhaseLockLoop<f32, P>, n_phase: f32) -> GflController<'a, f32, P> {
        GflController {
    ...
    /// Calculates the voltage dynamics, steps the states using the forward euler method, 
    /// and returns the output of the dVOC controller using the given input, u.
    /// # Arguments    
    /// * 'x' - PI integrator states as an array of values: (PI_i_d, PI_i_q)
    /// * 'u' - alpha-beta current (p.u.) and alpha-beta grid voltage (p.u.) as an array of values: (i_alpha, i_beta, vg_alpha, vg_beta)
    ...
    /// Calculates the output of the GFL controller using the given input, u.
    /// # Arguments    
...
impl<'a, const P: usize> Dynamics<f32, GFL_STATES, GFL_INPUTS> for GflController<'a, f32, P> {
    /// Calculates the voltage dynamics of the GFL controller using the given input, u.
    /// # Arguments    
    /// * 'x' - PI integrator states as an array of T fixed-point values: (PI_i_d, PI_i_q)
    /// * 'u' - alpha-beta current (p.u.) and alpha-beta grid voltage (p.u.) as an array of T fixed-point values: (i_alpha, i_beta, vg_alpha, vg_beta)
...
impl<'a, const P: usize> StepDynamics<f32, GFL_STATES, GFL_INPUTS> for GflController<'a, f32, P> {
    // Steps the dynamics using the forward euler method
    // # Arguments
